...
from src.reporting import write_stage2_reports


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run T.A.S.E Stage 2 controls.")
...
def main() -> None:
    ...
    results, patch_log, summary = run_stage2_controls(config)

    outputs_dir = ROOT / "outputs"
    outputs_dir.mkdir(parents=True, exist_ok=True)
    results.to_csv(outputs_dir / "results_by_seed_stage2.csv", index=False)
    patch_log.to_csv(outputs_dir / "patch_log_stage2.csv", index=False)
    summary.to_csv(outputs_dir / "summary_metrics_stage2.csv", index=False)
    technical_path, plain_path = write_stage2_reports(results, summary, config, ROOT / "reports")

    print(f"Wrote {outputs_dir / 'results_by_seed_stage2.csv'}")
    print(f"Wrote {outputs_dir / 'patch_log_stage2.csv'}")
    print(f"Wrote {outputs_dir / 'summary_metrics_stage2.csv'}")
