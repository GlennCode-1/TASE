...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the T.A.S.E synthetic kill-test.")
    parser.add_argument("--config", required=True, help="Path to killtest yaml config.")
...
def main() -> None:
    ...
    results, patch_log, summary = run_killtest(config)

    outputs_dir = ROOT / "outputs"
    outputs_dir.mkdir(parents=True, exist_ok=True)
    results.to_csv(outputs_dir / "results_by_seed.csv", index=False)
    patch_log.to_csv(outputs_dir / "patch_log.csv", index=False)
    summary.to_csv(outputs_dir / "summary_metrics.csv", index=False)
    technical_path, plain_path = write_reports(results, summary, config, ROOT / "reports")

    print(f"Wrote {outputs_dir / 'results_by_seed.csv'}")
    print(f"Wrote {outputs_dir / 'patch_log.csv'}")
    print(f"Wrote {outputs_dir / 'summary_metrics.csv'}")
